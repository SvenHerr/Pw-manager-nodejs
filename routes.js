...
export default function(app) {
  ...
  app.get('/', async function(req, res) {
    ...
    if (!req.session.loggedIn) {
      ...
      return res.render(
          'login', {errormsg : await sessionHandler.getErrorFromSession(req)});
    }
  ...
  app.post('/signup', async function(req, res) {
    try {
      if (await customer.signUp(req, res)) {
        ...
        return res.render(
            'signup',
            {userData : customer.getUserFromSession(req), errormsg : msg});
      }
    } catch (err) {
      console.log('Error on Login: ' + err);
      return res.render(
          'signup', {userData : user, errormsg : req.t('signup.generalError')});
    }
  });

  app.get('/signup', function(req, res) {
    return res.render(
        'signup', {userData : customer.getUserFromSession(req), errormsg : ''});
  });
  ...
  Object.keys(routes).forEach(function(path) {
    if (routes[path]?.routes?.post) {
      Object.keys(routes[path].routes?.post).forEach(function(method) {
        app.post(`/${path.toLowerCase()}/${method.toLowerCase()}`,
                 routes[path].routes?.post[method]);
      });
    }

    if (routes[path]?.routes?.get) {
      Object.keys(routes[path].routes?.get).forEach(function(method) {
        app.get(`/${path.toLowerCase()}/${method.toLowerCase()}`,
                routes[path].routes?.get[method]);
      });
    }
  });
