...
var administration =
    require('../Pw-manager-nodejs/administration'); // Change name!!!
...
app.post(
    "/getcustomers",
    async function(req, res) { await administration.getCustomers(req, res); });

app.post("/addnewpw",
         async function(req, res) { await administration.addNewPw(req, res); });

app.post("/copypw",
         async function(req, res) { await administration.copyPw(req, res); });

app.post("/changepw",
         async function(req, res) { await administration.changePw(req, res); });

app.post(
    "/changepwapp",
    async function(req, res) { await administration.changePwApp(req, res); });

app.get("/login", function(req, res) { res.render("login", {errormsg : ""}); });

app.post("/logout",
         async function(req, res) { await customer.logout(req, res); });

app.get("/logout",
        async function(req, res) { await customer.logout(req, res); });

app.post("/deletepw",
         async function(req, res) { await administration.deletePw(req, res); });
...
app.post("/signup", async function(req, res) {
  try {
    ...
    return res.render("signup",
                      {userData : user, errormsg : language.signUpError});
  }
});

app.post("/signin",
         async function(req, res) { await customer.signIn(req, res); });

app.post("/showpw",
         async function(req, res) { await administration.showPw(req, res); });
...
// here you set all routes that would end in cannot get/... or cannot post/...
// to default page could also be you own error page
